import{_ as n,o as s,c as a,e}from"./app-DMjJEOU4.js";const p={},t=e(`<h3 id="类型复用不足" tabindex="-1"><a class="header-anchor" href="#类型复用不足" aria-hidden="true">#</a> 类型复用不足</h3><p>大量的重复类型定义，这显著降低了代码的复用性。</p><p>许多团队成员并不清楚如何在TypeScript中复用类型。TypeScript允许我们使用type和interface来定义类型。</p><p>type与interface之间的区别时，大多数人都表示不清楚，这也就难怪他们不知道如何有效地复用类型了。</p><p>type定义的类型可以通过交叉类型（&amp;）来进行复用，而interface定义的类型则可以通过继承（extends）来实现复用。值得注意的是，type和interface定义的类型也可以互相复用。下面是一些简单的示例：</p><p>复用type定义的类型：</p><div class="language-typescript line-numbers-mode" data-ext="ts"><pre class="language-typescript"><code><span class="token keyword">type</span> <span class="token class-name">Point</span> <span class="token operator">=</span> <span class="token punctuation">{</span>
